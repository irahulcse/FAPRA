...
public class SwitchingDecision {
    ...
    /**
     * Total time consumption = network time + instantiation time + sync time (by parallel PET enforcement) + switching time.
     * The pending time ({@link #switchBegin} - {@link #instantiationEnd}) , during which the operator waits for the right element to arrive, doesn't contribute to
     * the total time consumption of switching procedure.
     *
     * @return
     */
/*    public String getTotalTimeConsumption() {
        if (getSwitchingTimeConsumption().equals("-")) {
            return "-";
        } else {
            if (switchEnd == null || switchBegin == null) return "-";
            if (syncEnd == null) return String.valueOf(instantiationEnd - generationTime + switchEnd - switchBegin);
            return String.valueOf(Long.parseLong(getSwitchingTimeConsumption()) + Long.parseLong(getSyncDelay()) + Long.parseLong(getInstantiationTimeConsumption()) + Long.parseLong(getNetworkDelay()));
        }

        //return String.valueOf(syncEnd - generationTime + switchEnd - switchBegin);
    }*/
    ...
    public String getTotalTimeConsumption() {
        switch (evaluationMode) {
            ...
            default -> {
                throw new IllegalArgumentException("Cannot handle the evaluation mode: " + evaluationMode);
            }
        }
    }
}
